...
class 求解器类:
    ...
    def 计算成功概率(self) -> list[tuple[int, int, int, int, int, float]]:
        ...
        for 初始源石锭数量 in range(0, self.电表倒转成功源石锭阈值 + 1, 1):
            for 烛火数量 in range(1, 6 + 1, 1):
                状态 = 过渡态类(源石锭数量=初始源石锭数量, 烛火数量=烛火数量)
                状态序号 = self.状态索引[状态]
                成功概率 = 最小二乘解[状态序号]
                结果列表.append((self.电表倒转成功源石锭阈值, self.衡钱数量, self.厉钱数量, 初始源石锭数量, 烛火数量, 成功概率))
        return 结果列表


def 单个衡钱和厉钱数量计算(电表倒转成功源石锭阈值: int, 衡钱数量: int, 厉钱数量: int) -> list[tuple[int, int, int, int, int, float]]:
    求解器 = 求解器类(电表倒转成功源石锭阈值=电表倒转成功源石锭阈值, 衡钱数量=衡钱数量, 厉钱数量=厉钱数量)
    求解器.构建状态列表()
    求解器.构建状态转移矩阵()
    print(f"电表倒转成功源石锭阈值：{电表倒转成功源石锭阈值}，衡钱数量：{衡钱数量}，厉钱数量：{厉钱数量}，状态转移矩阵构建完成，状态数量：{求解器.状态数量}")
    结果列表 = 求解器.计算成功概率()
    print(f"电表倒转成功源石锭阈值：{电表倒转成功源石锭阈值}，衡钱数量：{衡钱数量}，厉钱数量：{厉钱数量}，最小二乘解计算完成")
    return 结果列表
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(结果列表, columns=["电表倒转成功源石锭阈值", "衡钱数量", "厉钱数量", "初始源石锭数量", "初始烛火数量", "成功概率"])
    df.to_csv("游客分队电表倒转成功概率.csv.gz", index=False, compression="gzip")
