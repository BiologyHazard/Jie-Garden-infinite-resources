...
class 求解器类:
    ...
    def 计算成功概率(self) -> list[tuple[int, int, int, float]]:
        ...
        for 初始源石锭数量 in range(0, self.电表倒转成功源石锭阈值 + 1, 1):
            状态 = 过渡态类(源石锭数量=初始源石锭数量, 烛火数量=1, 票券可投钱次数=0)
            状态序号 = self.状态索引[状态]
            成功概率 = 最小二乘解[状态序号]
            结果列表.append((self.厉钱数量, self.每次投钱消耗票券数量, 初始源石锭数量, 成功概率))
        return 结果列表


def 单个厉钱数量计算(电表倒转成功源石锭阈值: int, 厉钱数量: int, 每次投钱消耗票券数量: int) -> list[tuple[int, int, int, float]]:
    求解器 = 求解器类(电表倒转成功源石锭阈值=电表倒转成功源石锭阈值, 厉钱数量=厉钱数量, 每次投钱消耗票券数量=每次投钱消耗票券数量)
    求解器.构建状态列表()
    求解器.构建状态转移矩阵()
    print(f"状态转移矩阵构建完成，状态数量：{求解器.状态数量}")
    结果列表 = 求解器.计算成功概率()
    for 厉钱数量, 每次投钱消耗票券数量, 初始源石锭数量, 成功概率 in 结果列表:
        print(f"后勤分队，厉钱数量：{厉钱数量}，每次投钱消耗票券数量：{每次投钱消耗票券数量}，初始源石锭数量：{初始源石锭数量}，成功概率：{成功概率:.4f}")
    return 结果列表


if __name__ == "__main__":
    # 求解器 = 求解器类(电表倒转成功源石锭阈值=1024, 厉钱数量=9, 每次投钱消耗票券数量=2)
    # 求解器.构建状态列表()
    # 求解器.构建状态转移矩阵()
    # print(f"状态转移矩阵构建完成，状态数量：{求解器.状态数量}")
    # # print(求解器.迭代计算成功概率(512, 512))
    # 结果列表 = 求解器.计算成功概率()
    # exit()

    电表倒转成功源石锭阈值 = 4096
    待计算的厉钱数量范围 = range(5, 9 + 1)
    待计算的每次投钱消耗票券数量范围 = (1, 2)

    with multiprocessing.Pool(processes=len(待计算的厉钱数量范围) * len(待计算的每次投钱消耗票券数量范围)) as pool:
        results = pool.starmap(
            单个厉钱数量计算,
            ((电表倒转成功源石锭阈值, 厉钱数量, 每次投钱消耗票券数量)
             for 厉钱数量 in 待计算的厉钱数量范围
             for 每次投钱消耗票券数量 in 待计算的每次投钱消耗票券数量范围)
        )
    ...
    df = pd.DataFrame(结果列表, columns=["厉钱数量", "每次投钱消耗票券数量", "初始源石锭数量", "电表倒转成功概率"])
    df.to_csv("后勤分队电表倒转成功概率.csv", index=False)
